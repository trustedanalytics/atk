...
class LdaScoreModelTest extends FlatSpec with Matchers with ScalaFutures {
  ...
  "predict" should "compute topic probabilities for document" in {
    ...
    score0(0).asInstanceOf[Vector[Double]].toArray should equalWithTolerance(Array(0.5, 0.333333))
    score0(1).asInstanceOf[Int] should equal(1)
    score0(2).asInstanceOf[Double] should equal(100 / 6d +- epsilon)

    score1(0).asInstanceOf[Vector[Double]].toArray should equalWithTolerance(Array(0.4375, 0.5625))
    score1(1).asInstanceOf[Int] should equal(0)
    score1(2).asInstanceOf[Double] should equal(0d +- epsilon)

    score2(0).asInstanceOf[Vector[Double]].toArray should equalWithTolerance(Array(0d, 0d))
    score2(1).asInstanceOf[Int] should equal(0)
    score2(2).asInstanceOf[Double] should equal(0d +- epsilon)
