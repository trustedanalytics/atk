...
class LdaScoreModelTest extends FlatSpec with Matchers with ScalaFutures {
  ...
  "predict" should "compute topic probabilities for document" in {
    ...
    val score0 = scores(0).asInstanceOf[Map[String, Any]]
    val score1 = scores(1).asInstanceOf[Map[String, Any]]
    val score2 = scores(2).asInstanceOf[Map[String, Any]]

    //val score01 = score0(1).asInstanceOf[Map[String, Any]]
    score0("topics_given_doc").asInstanceOf[List[Double]].toArray should equalWithTolerance(Array(0.5, 0.333333))
