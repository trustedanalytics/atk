...
class MAXScoreModel(maxModel: MAXModel, maxData: MAXData)
    extends MAXModel(maxModel.p, maxModel.d, maxModel.q, maxModel.xregMaxLag, maxModel.coefficients,
      maxModel.includeOriginalXreg, maxModel.includeIntercept) with Model {

  override def score(data: Array[Any]): Array[Any] = {
    val xColumnsLength = maxData.xColumns.length
  ...
  override def modelMetadata(): ModelMetaDataArgs = {
    new ModelMetaDataArgs("MAX Model", classOf[MAXModel].getName, classOf[MAXModelReaderPlugin].getName, Map())
  }
