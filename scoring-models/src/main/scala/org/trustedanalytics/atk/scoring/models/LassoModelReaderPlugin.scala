...
class LassoModelReaderPlugin() extends ModelLoader {

  private var model: LassoScoreModel = _

  override def load(bytes: Array[Byte]): Model = {
    val str = new String(bytes)
    println(str)
    val json: JsValue = str.parseJson
    val lassoData = json.convertTo[LassoData]
    model = new LassoScoreModel(lassoData)
