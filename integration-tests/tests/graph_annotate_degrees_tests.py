...
class GraphAnnotateDegreesTest(unittest.TestCase):
    ...
    def test_annotate_degrees(self):
        ...
        graph.define_edge_type("labeldirected", "primary", "primary", directed=True)
        graph.define_edge_type("undirected", "primary", "primary", directed=False)
        ...
        graph.edges['undirected'].add_edges(undirected_frame, "node1",
                                            "node2", ["value"])
        output = graph.annotate_degrees("countName", degree_option="in")
        self.assertTrue(type(output) is dict)
        self.assertTrue(output.has_key('primary'))
        frame_parquet = output['primary']
        self.assertTrue(dict(frame_parquet.schema).has_key('countName'))
