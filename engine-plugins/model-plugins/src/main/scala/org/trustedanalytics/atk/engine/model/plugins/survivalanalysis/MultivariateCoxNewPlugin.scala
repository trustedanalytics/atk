...
@PluginDoc(oneLine = "Create a 'new' instance of a Multivariate Cox Proportional Hazards model",
  extended = """ """)
class MultivariateCoxNewPlugin extends CommandPlugin[GenericNewModelArgs, ModelReference] {
  /**
   ...
   */
  override def name: String = "model:multivariate_cox/new"

  override def execute(arguments: GenericNewModelArgs)(implicit invocation: Invocation): ModelReference = {
    engine.models.createModel(CreateEntityArgs(name = arguments.name, entityType = Some("model:multivariate_cox")))
  }
}
