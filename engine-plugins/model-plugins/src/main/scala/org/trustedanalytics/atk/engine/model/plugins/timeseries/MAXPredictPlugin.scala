...
class MAXPredictPlugin extends SparkCommandPlugin[MAXPredictArgs, FrameReference] {
   ...
   */
  override def name: String = "model:max/predict"
  ...
  override def numberOfJobs(arguments: MAXPredictArgs)(implicit invocation: Invocation) = 1
   ...
   */
  override def execute(arguments: MAXPredictArgs)(implicit invocation: Invocation): FrameReference = {
    val frame: SparkFrame = arguments.frame
    val model: Model = arguments.model

    //Extracting the MAXModel from the stored JsObject
    val maxData = model.data.convertTo[MAXData]
    val maxModel = maxData.maxModel

    require(maxData.xColumns.length == arguments.xColumns.length, "Number of columns for train and predict should be the same")
    ...
    if (predictions.length != frame.rdd.count)
      throw new RuntimeException("Received unexpected number of y values from MAX Model predict (expected " +
    ...
    engine.frames.tryNewFrame(CreateEntityArgs(description = Some("created by MAXModel predict command"))) {
