...
/**
 * Publish a MAX Model for scoring
 */
...
class MAXPublishPlugin extends CommandPlugin[ModelPublishArgs, ExportMetadata] {
   ...
   */
  override def name: String = "model:max/publish"
  ...
  override def execute(arguments: ModelPublishArgs)(implicit invocation: Invocation): ExportMetadata = {
    ...
    //Extracting the MAXData from the stored JsObject
    val maxData = model.data.convertTo[MAXData]
    val jsvalue: JsValue = maxData.toJson

    val modelartifacts = ModelPublish.createTarForScoringEngine(jsvalue.toString().getBytes(Charsets.UTF_8), "scoring-models", "org.trustedanalytics.atk.scoring.models.MAXModelReaderPlugin")

    ExportMetadata(modelartifacts.filePath, "model", "tar", modelartifacts.fileSize, model.name.getOrElse("max_model"))
  }
}
