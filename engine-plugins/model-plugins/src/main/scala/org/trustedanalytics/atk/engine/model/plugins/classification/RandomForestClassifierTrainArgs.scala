...
case class RandomForestClassifierTrainArgs(@ArgDoc("""Handle to the model to be used.""") model: ModelReference,
                                           ...
                                           @ArgDoc("""Criterion used for information gain calculation.
Supported values "gini" or "entropy"""") impurity: String = "gini",
                                           @ArgDoc("""Maximum depth of the tree.""") maxDepth: Int = 4,
                                           @ArgDoc("""Maximum number of bins used for
splitting features.""") maxBins: Int = 100,
                                           @ArgDoc("""Random seed for bootstrapping and
choosing feature subsets""") seed: Int = scala.util.Random.nextInt(),
                                           @ArgDoc("""Arity of categorical features.
Entry (n-> k) indicates that feature 'n' is categorical with 'k' categories indexed
from 0:{0,1,...,k-1}""") categoricalFeaturesInfo: Option[Map[Int, Int]] = None,
                                           @ArgDoc("""Number of features to consider for splits at each node.
Supported values\: "auto","all","sqrt","log2","onethird".""") featureSubsetCategory: Option[String] = None) {
