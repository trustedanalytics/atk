...
import org.trustedanalytics.atk.scoring.models.{ LassoModelReaderPlugin, LassoData }
...
/**
 * Publish a Lasso Model for scoring
 */
@PluginDoc(oneLine = "Creates a tar file that will be used as input to the scoring engine",
  extended = """The publish method exports the LassoModel and its implementation into a tar file.
...
class LassoPublishPlugin extends CommandPlugin[ModelPublishArgs, ExportMetadata] {
   ...
   */
  override def name: String = "model:lasso/publish"
  ...
  override def execute(arguments: ModelPublishArgs)(implicit invocation: Invocation): ExportMetadata = {
    ...
    //Extracting the LassoModel from the stored JsObject
    val lassoData = model.data.convertTo[LassoData]
    val jsvalue: JsValue = lassoData.toJson

    val modelArtifact = ModelPublish.createTarForScoringEngine(jsvalue.toString().getBytes(Charsets.UTF_8), "scoring-models", classOf[LassoModelReaderPlugin].getName)
    ExportMetadata(modelArtifact.filePath, "model", "tar", modelArtifact.fileSize, model.name.getOrElse("lasso_model"))
  }
}
