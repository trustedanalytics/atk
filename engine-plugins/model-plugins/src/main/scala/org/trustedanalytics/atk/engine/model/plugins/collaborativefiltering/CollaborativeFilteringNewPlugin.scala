 ...
 */
@PluginDoc(oneLine = "Create a new Collaborative Filtering (ALS) model.",
  extended = """For details about Collaborative Filter (ALS) modelling,
...
class CollaborativeFilteringNewPlugin extends CommandPlugin[GenericNewModelArgs, ModelReference] {
  ...
  override def execute(arguments: GenericNewModelArgs)(implicit invocation: Invocation): ModelReference = {
    ...
    models.createModel(CreateEntityArgs(name = arguments.name, entityType = Some("model:collaborativefiltering/new")))
  }
}
