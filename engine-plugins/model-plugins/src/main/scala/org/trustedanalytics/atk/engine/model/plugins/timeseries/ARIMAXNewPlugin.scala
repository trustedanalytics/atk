...
@PluginDoc(oneLine = "Create a 'new' instance of an Autoregressive Integrated Moving Average with Explanatory Variables (ARIMAX) model.",
  extended =
    """ Regression with ARMA errors - ARIMAX [1]
      |[1] http://robjhyndman.com/hyndsight/arimax/
    """.stripMargin,
  returns = "A new instance of ARIMAXModel")
class ARIMAXNewPlugin extends CommandPlugin[GenericNewModelArgs, ModelReference] {
  override def name: String = "model:arimax/new"

  override def execute(arguments: GenericNewModelArgs)(implicit invocation: Invocation): ModelReference = {
    engine.models.createModel(CreateEntityArgs(name = arguments.name, entityType = Some("model:arimax")))
  }
}
