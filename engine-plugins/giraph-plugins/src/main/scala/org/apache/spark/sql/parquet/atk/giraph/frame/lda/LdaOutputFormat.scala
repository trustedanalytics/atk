...
class LdaParquetFrameVertexWriter(conf: LdaConfiguration,
  ...
  //TODO: Save string or int ID
  private def getVertexId(vertex: Vertex[LdaVertexId, LdaVertexData, Nothing]): String = {
    if (vertex.getValue.hasAutoGeneratedId) {
      vertex.getValue.getDescription
    }
    else {
      vertex.getId.getValue.toString
    }
  }

  private def giraphLdaResultToRow(vertex: Vertex[LdaVertexId, LdaVertexData, Nothing]): Row = {
    val content = new Array[Any](2)
    content(0) = getVertexId(vertex)
