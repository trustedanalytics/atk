...
class DaalPrincipalComponentsPublishPlugin extends CommandPlugin[ModelPublishArgs, ExportMetadata] {
  ...
  override def execute(arguments: ModelPublishArgs)(implicit invocation: Invocation): ExportMetadata = {
    ...
    //TODO: Update scoring methods once DAAL supports linear algebra operations on its numeric tables
    ...
    ExportMetadata(modelArtifact.filePath, "model", "tar", modelArtifact.fileSize,
      model.name.getOrElse("daal_principal_components_model"))
  }
}
