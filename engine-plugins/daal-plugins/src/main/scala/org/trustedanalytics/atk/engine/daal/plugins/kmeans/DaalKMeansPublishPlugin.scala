...
import org.trustedanalytics.atk.scoring.models.DaalKMeansModelReaderPlugin
...
/**
 * Publish an Intel DAAL KMeansModel for scoring
 */
...
class DaalKMeansPublishPlugin extends CommandPlugin[ModelPublishArgs, ExportMetadata] {
  ...
  override def execute(arguments: ModelPublishArgs)(implicit invocation: Invocation): ExportMetadata = {
    ...
    val modelartifacts = ModelPublish.createTarForScoringEngine(jsvalue.toString().getBytes(Charsets.UTF_8),
      "scoring-models", classOf[DaalKMeansModelReaderPlugin].getName,
      Some(DaalUtils.getDaalLibraryPaths(EngineConfig.daalDynamicLibraries)))

    ExportMetadata(modelartifacts.filePath, "model", "tar", modelartifacts.fileSize, model.name.getOrElse("daal_kmeans_model"))
  }
}
