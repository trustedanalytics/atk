...
class FrameBackendRest(object):
    ...
    def get_row_count(self, frame, where):
        ...
        arguments = {'frame': self.get_ta_uri(frame),
                     'udf': get_udf_arg(frame, where, icountwhere)}
        return executor.execute("frame/count_where", self, arguments)


    def get_ta_uri(self, frame):
        return self._get_frame_info(frame).ta_uri
...
status = {status}""".format(type=frame_type, name=frame_name, graph_data=graph_data, row_count=row_count, schema=schema, status=status)
    ...
    def join(self, left, right, left_on, right_on, how, name=None):
        ...
        arguments = {"name": name,
                     "how": how,
                     "left_frame": {"frame": self.get_ta_uri(left), "join_column": left_on},
                     "right_frame": {"frame": self.get_ta_uri(right), "join_column": right_on} }
    ...
    def initialize_graph_frame(self, frame, frame_info, graph):
        ...
        frame._ta_uri = frame_info.ta_uri
...
def become_frame(frame, source_frame):
    ...
    frame._ta_uri = source_frame._ta_uri
